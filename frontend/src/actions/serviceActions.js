...
export const listServiceCategories = () => async (dispatch) => {
  ...
  try {
    const { data } = await Axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/services/categories`);
...
export const updateService = (service) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/services/${service._id}`, service, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
...
export const deleteService = (serviceId) => async (dispatch, getState) => {
  ...
  try {
    const { data } = Axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/services/${serviceId}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
