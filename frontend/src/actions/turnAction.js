...
export const updateTurn = (id) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/turn/${id}`, {
...
export const deleteTurn = (id) => async (dispatch, getState) => {
  ...
  try {
    const { data } = Axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/turn/${id}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
...
export const getTurn = (id) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/turn/${id}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
