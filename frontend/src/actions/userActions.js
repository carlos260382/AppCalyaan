...
export const register =
	(name, email, password, numberPhone) => async dispatch => {
		dispatch({
			type: USER_REGISTER_REQUEST,
			payload: { email, password, numberPhone },
		});
		try {
...
export const signin = (email, password) => async dispatch => {
	...
	try {
		const { data } = await Axios.post(
			`${process.env.REACT_APP_API_BASE_URL}/api/users/signin`,
			{ email, password }
		);
		dispatch({ type: USER_SIGNIN_SUCCESS, payload: data });
		localStorage.setItem('userInfo', JSON.stringify(data));
...
export const updateUser = user => async (dispatch, getState) => {
	...
	try {
		const { data } = await Axios.put(
			`${process.env.REACT_APP_API_BASE_URL}/api/users/${user._id}`,
			user,
			{
				headers: { Authorization: `Bearer ${userInfo.token}` },
			}
...
export const listTopSellers = () => async dispatch => {
	...
	try {
		const { data } = await Axios.get(
			`${process.env.REACT_APP_API_BASE_URL}/api/users/top-sellers`
		);
